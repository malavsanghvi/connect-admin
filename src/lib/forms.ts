...
import { fromDateTimeLocal, parseDollarsToCents } from "@/lib/format";
import { fail, reportError, type ActionResult } from "@/lib/result";
 ...
 */
export async function runAction(context: string, doing: string, fn: () => Promise<ActionResult<unknown>>): Promise<ActionResult<unknown>> {
  try {
    return await fn();
  } catch (error) {
    unstable_rethrow(error);
    if (error instanceof FormError) return fail(error.message);
    if (error instanceof DbFailure) return reportError(context, error.dbError, error.doing);
    return reportError(context, error, doing);
  }
}
