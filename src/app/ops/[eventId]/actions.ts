...
import { FormError, must, runAction } from "@/lib/forms";
...
function under12(dob: string | null, today: string) {
  if (!dob) return false;
...
function senior(dob: string | null, today: string) {
  if (!dob) return false;
  return Number(today.slice(0, 4)) - Number(dob.slice(0, 4)) >= 65;
}
...
export async function findWalkIn(input: { eventId: string; phone: string }): Promise<ActionResult<WalkInLookup>> {
  return runAction("ops.findWalkIn", "look up that number", async () => {
    const { supabase, viewer } = await actionContext((a) => areas.checkIn(a, input.eventId), "You're not a check-in volunteer for this event.");
    const phone = toE164(input.phone);
    if (!phone) throw new FormError("Enter a mobile number with area code, e.g. (713) 555-0198.");

    const rsvps = must(
      await supabase.from("rsvps").select("id, household_id, guest_name, status").eq("event_id", input.eventId).eq("guest_phone_e164", phone),
      "look up RSVPs for that number",
    ) ?? [];

    const canSearchMembers = can(viewer.access, "people.view");
    const households: WalkInLookup["households"] = [];
    const householdRsvps = new Map<string, string>();
    if (canSearchMembers) {
      const people = must(
        await supabase.from("people").select("id").eq("center_id", viewer.center.id).eq("phone_e164", phone).is("merged_into_id", null),
        "look up members with that number",
      ) ?? [];
      if (people.length) {
        const links = must(
          await supabase.from("household_members").select("household_id").in("person_id", people.map((p) => p.id)).is("left_at", null),
          "find their households",
        ) ?? [];
        const hhIds = [...new Set(links.map((l) => l.household_id))];
        if (hhIds.length) {
          const [hhs, members, hhRsvps] = await Promise.all([
            supabase.from("households").select("id, display_name").in("id", hhIds),
            supabase.from("household_members").select("household_id, person_id, is_primary").in("household_id", hhIds).is("left_at", null),
            supabase.from("rsvps").select("id, household_id, status").eq("event_id", input.eventId).in("household_id", hhIds).neq("status", "cancelled"),
          ]);
          ...
          const peopleRows = m.length
            ? (must(
                await supabase.from("people").select("id, first_name, last_name, preferred_name, date_of_birth, is_deceased").in("id", m.map((x) => x.person_id)),
                "load family names",
              ) ?? [])
            : [];
            ...
            households.push({
              ...
              members: m
                ...
                .map((p) => ({
                  ...
                  child: under12(p.date_of_birth, today),
                  senior: senior(p.date_of_birth, today),
                  assistance: false,
                })),
            });
          }
    ...
    const attendees = rsvpIds.length
      ? (must(await supabase.from("attendees").select("rsvp_id, display_name, ticket_token, ticket_revoked").in("rsvp_id", rsvpIds), "load the party") ?? [])
      : [];
    const allRsvps = rsvpIds.length
      ? (must(await supabase.from("rsvps").select("id, household_id, guest_name, status").in("id", rsvpIds), "load RSVPs") ?? [])
      : [];
    ...
    return ok(undefined, { phone, existing, households, canSearchMembers });
  }) as Promise<ActionResult<WalkInLookup>>;
}
...
export async function registerWalkIn(input: {
  ...
  return runAction("ops.registerWalkIn", "register the walk-in", async () => {
    const { supabase, centerId } = await actionContext((a) => areas.checkIn(a, input.eventId), "You're not a check-in volunteer for this event.");
    ...
    if (!token) throw new FormError("The walk-in was saved but no ticket was issued. Scan again or use the Events page.");
    return ok("Walk-in registered.", { token, attendeeIds: created.map((a) => a.id) });
  }) as Promise<ActionResult<{ token: string; attendeeIds: string[] }>>;
}
