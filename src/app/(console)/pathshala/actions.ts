...
import { areas, can, hasScopedRole, type Access } from "@/lib/access";
...
// Terms
// ---------------------------------------------------------------------------
export async function saveTerm(termId: string | null, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.saveTerm", "save the term", async () => {
    const { supabase, centerId, tz } = await actionContext(areas.pathshalaManage, "Only the Pathshala principal can change terms.");
...
// ---------------------------------------------------------------------------
export async function saveClass(classId: string | null, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.saveClass", "save the class", async () => {
    const { supabase, centerId } = await actionContext(areas.pathshalaManage, "Only the Pathshala principal can change classes.");
...
export async function addTeacher(classId: string, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.addTeacher", "add the teacher", async () => {
    const { supabase, centerId, viewer } = await actionContext(areas.pathshalaManage, "Only the Pathshala principal can assign teachers.");
    ...
    const note = await syncTeacherGrant(supabase, viewer.access, centerId, classId, personId, "grant", viewer.userId);
    refresh();
    return ok(`Teacher added.${note ? " " + note : ""}`);
  });
}

export async function removeTeacher(teacherRowId: string, fd: FormData): Promise<ActionResult<unknown>> {
  void fd;
  return runAction("pathshala.removeTeacher", "remove the teacher", async () => {
    const { supabase, centerId, viewer } = await actionContext(areas.pathshalaManage, "Only the Pathshala principal can remove teachers.");
    ...
    const note = await syncTeacherGrant(supabase, viewer.access, centerId, row.class_id, row.person_id, "revoke", viewer.userId);
    refresh();
...
async function syncTeacherGrant(
  ...
  if (!cu) return "They haven't signed in to Connect yet, so their class access will need to be granted after they do.";
  if (mode === "grant") {
...
// Enrollments
// ---------------------------------------------------------------------------
async function seatsTaken(supabase: ServerSupabase, classId: string): Promise<number> {
...
export async function placeEnrollment(enrollmentId: string, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.placeEnrollment", "place the student", async () => {
    const { supabase } = await actionContext(areas.pathshalaManage, "Only the Pathshala principal can place students.");
...
export async function waitlistEnrollment(enrollmentId: string, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.waitlistEnrollment", "waitlist the student", async () => {
    const { supabase } = await actionContext(areas.pathshalaManage, "Only the Pathshala principal can manage the waitlist.");
...
export async function setEnrollmentStatus(enrollmentId: string, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.setEnrollmentStatus", "update the enrollment", async () => {
    const { supabase } = await actionContext(areas.pathshalaManage, "Only the Pathshala principal can change enrollments.");
...
export async function saveEnrollmentNote(enrollmentId: string, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.saveEnrollmentNote", "save the note", async () => {
    const { supabase } = await actionContext(areas.pathshalaManage, "Only the Pathshala principal can edit enrollment notes.");
...
// ---------------------------------------------------------------------------
function canTakeAttendance(access: Access, classId: string) {
  return hasScopedRole(access, classId, "teacher") || can(access, "pathshala.manage");
}

async function ensureSession(supabase: ServerSupabase, centerId: string, classId: string, heldOn: string, userId: string) {
  const find = async () =>
      ...
      await supabase.from("pathshala_attendance").upsert(
        {
          ...
          marked_via: hasScopedRole(viewer.access, input.classId, "teacher") && !viewer.access.isPlatformAdmin ? "teacher" : "admin",
...
export async function markAllPresent(input: { classId: string; heldOn: string; enrollmentIds: string[] }): Promise<ActionResult<unknown>> {
  return runAction("pathshala.markAllPresent", "mark everyone present", async () => {
    ...
    const via = hasScopedRole(viewer.access, input.classId, "teacher") && !viewer.access.isPlatformAdmin ? "teacher" : "admin";
    must(
      await supabase.from("pathshala_attendance").upsert(
...
export async function saveSessionTopic(classId: string, heldOn: string, fd: FormData): Promise<ActionResult<unknown>> {
...
// Gyan Path sign-offs
// ---------------------------------------------------------------------------
export async function decideSignoff(signoffId: string, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.decideSignoff", "record the sign-off", async () => {
    ...
    // The main button carries no value (approve); "Needs work" is a second submit button.
    const decision = oneOf(fd, "decision", ["approved", "needs_work"] as const, "Decision", "approved");
    const note = str(fd, "note");
    if (decision === "needs_work" && !note) throw new FormError("Add a short note so the student knows what to practise.");
    const updated = must(
      await supabase
        .from("gyan_signoffs")
    ...
    refresh();
    return ok(decision === "approved" ? "Signed off." : "Sent back with your note.");
  });
}
...
// Class announcements
// ---------------------------------------------------------------------------
export async function saveAnnouncement(announcementId: string | null, fd: FormData): Promise<ActionResult<unknown>> {
  return runAction("pathshala.saveAnnouncement", "save the announcement", async () => {
    ...
    if (!classId && !can(viewer.access, "pathshala.manage")) {
      throw new FormError("Only the Pathshala principal can post to the whole Pathshala. Choose one of your classes.");
    }
    if (classId && !can(viewer.access, "pathshala.manage") && !hasScopedRole(viewer.access, classId, "teacher")) {
...
export async function setAnnouncementPublished(announcementId: string, fd: FormData): Promise<ActionResult<unknown>> {
...
export async function deleteAnnouncement(announcementId: string, fd: FormData): Promise<ActionResult<unknown>> {
  void fd;
