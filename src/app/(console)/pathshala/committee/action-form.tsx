    ...
    >
      ...
      <FormGrid>
        <Field label="Action" className="sm:col-span-2">
          <input name="name" required defaultValue={action?.name ?? ""} className="field-input" />
        </Field>
        <Field label="Event" hint="Leave empty for a standalone action">
          <Select
            name="event_id"
        ...
        <Field label="Phase" hint="“During” actions always use the event date">
          <Select
            name="phase"
        ...
        <PersonPicker name="owner_person_id" label="Owner" initial={owner ? [owner] : []} />
        <PersonPicker name="backup_owner_ids" label="Backups" multiple initial={backups} />
        <Field label="Due date">
          <input type="date" name="due_on" defaultValue={action?.due_on ?? ""} className="field-input" />
        </Field>
        ...
        <Field label="Details" className="sm:col-span-2">
          <textarea name="description" rows={3} defaultValue={action?.description ?? ""} className="field-input" />
        </Field>
      </FormGrid>
