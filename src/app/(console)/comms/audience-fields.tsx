...
import { presetOf, type AudiencePreset } from "@/lib/logic/audience";

type Opt = { id: string; name: string };

/** Audience presets for comms: all members, zones, Pathshala parents by class, event RSVPs, or custom JSON. */
    ...
    <fieldset>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {presets.map((p) => (
          <label
            key={p.key}
            className={`flex min-h-12 cursor-pointer items-start gap-3 rounded-lg border-2 p-3 ${preset === p.key ? "border-navy bg-navy-soft" : "border-line bg-white"}`}
          >
      ...
      {preset === "event_rsvps" && (
        <div className="mt-3 space-y-2">
          <select name="event_id" defaultValue={typeof init.event_id === "string" ? init.event_id : ""} className="field-input" aria-label="Event">
